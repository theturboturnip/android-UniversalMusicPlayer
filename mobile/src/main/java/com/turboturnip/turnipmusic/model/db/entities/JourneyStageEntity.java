...
public class JourneyStageEntity {
	...
	public JourneyStageEntity(Journey.Stage base){
		...
		this.filterArrayJSON = JSONHelper.encodeStringArray(base.filters);
	}
	...
	public final MusicFilter[] getFilters(){
		if (filterArrayJSON == null || filterArrayJSON.isEmpty()) return new MusicFilter[0];
		try {
			String[] filterStrings = JSONHelper.decodeStringArray(filterArrayJSON);
			MusicFilter[] filters = new MusicFilter[filterStrings.length];
			for (int i = 0; i < filters.length; i++){
				filters[i] = new MusicFilter(filterStrings[i]);
			}
			return filters;
		}catch(JSONException e){
			e.printStackTrace();
